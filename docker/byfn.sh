...
function printHelp () {
  ...
  echo "      - 'start' - bring up the network with docker-compose start"
  echo "      - 'stop' - clear the network with docker-compose stop"
  echo "      - 'clean' - delete the generated file"
...
function networkUp () {
  ...
  #init mysql
  docker exec mysql_5.7 bash /init/db_init.sh
}
...
function network() {
  ...
  cd network_configs
  cp network-config-tls-template.yaml network-config-tls.yaml
  ...
  cd "$CURRENT_DIR"
  sed $OPTS "s/ORG1_PRIVATE_KEY/${PRIV_KEY}/g" network-config-tls.yaml

  # If MacOSX, remove the temporary backup of the docker-compose file
  if [ "$ARCH" == "Darwin" ]; then
    rm network-config-tls.yamlt
  fi
}
...
function start (){
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH start
  else
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE start
  fi
}
