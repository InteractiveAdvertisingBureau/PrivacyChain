...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init","acxiom"]}' -P "OR	('Org1MSP.peer','Org2MSP.peer')" >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -l ${LANGUAGE} -v 1.0 -c '{"Args":["init","acxiom"]}' -P "OR	('Org1MSP.peer','Org2MSP.peer')" >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
...
upgradeChaincode () {
    ...
    set -x
    peer chaincode upgrade -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -v 2.0 -c '{"Args":["init","acxiom"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer','Org3MSP.peer')"
    res=$?
	set +x
    cat log.txt
...
chaincodeQuery () {
  ...
  do
     ...
     set -x
     peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["get","init"]}' >&log.txt
	 res=$?
     set +x
...
chaincodeInvoke () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -c '{"Args":["set","init","acxiom"]}' >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -c '{"Args":["set","init","acxiom"]}' >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
